...
const seedStuff = async ()=> {
    try {
        const seed = await pool.query(`INSERT INTO cohorts (cohort_number, start, graduation, instructor) VALUES 
        (19, '01/17/2023', '05/26/2023', 'Phil Witkin'),
        (20, '03/27/2023', '07/21/2023', 'Danny Andrews')`, (err)=>{
            if (err) {
                console.log(err);
            }
...
const cohortData = async function() {
    try {
        ...
        (20, '03/27/2023', '07/21/2023', 'Danny Andrews')`, (err)=>{
            ...
            console.log('Cohorts Table successfully seeded.')
            pool.query(`INSERT INTO students (stu_name, email, github, cohort_id) VALUES 
            ...
            ('Shane Metz', 'Smetzyboi88@yahoo.com', 'https://github.com/Rocio55', 2)`, (err)=>{
                if (err) {
                    console.log(err);
                }
                console.log('Students Table successfully seeded.')
                pool.query(`INSERT INTO assessments (assess_name, type) VALUES 
                ...
                ('Arrays', 'Checkpoint'),
                ('Objects', 'Checkpoint'),
                ('React Assessment', 'Assessment')`, (err)=>{
                    if (err) {
                        console.log(err);
                    }
                    console.log('Assessments Table successfully seeded.')
                    pool.query(`INSERT INTO projects (project_name, type) VALUES 
                    ...
                    ('Blue Ocean', 'Group')`, (err)=>{
                        if (err) {
                            console.log(err);
                        }
                        console.log('Projects Table successfully seeded.')
                        pool.query(`INSERT INTO groups (group_name, student1, student2, student3, student4, student5, student6) VALUES 
                        ('Yoshi''s Angels', 'David Ortiz', 'Billy Tomasello', 'Jesthen Baez', 'Shuyi Hoo', 'Shawn Couch', 'Tomas Corradini'),
                        ('The 20-Somethings', 'Salvatore Paucek', 'Clyde Welch', 'Sonya Hermann', 'Scott Bogan', null, null)`, (err)=>{
                            if (err) {
                                console.log(err);
                            }
                            console.log('Groups Table successfully seeded.')
                            pool.query(`INSERT INTO assessment_scores (student_id, assess_id, grade, cohort_id) VALUES 
                            (2, 4, 100, 1),
                            (3, 5, 100, 1),
                            (4, 6, 100, 1),
                            (7, 4, 90, 2),
                            ...
                            (9, 6, 90, 2)`, (err)=>{
                                ...
                                pool.query(`INSERT INTO project_scores (group_id, project_id, grade, cohort_id) VALUES 
                                (1, 9, 100, 1),
                                (1, 8, 95, 1),
                                (1, 7, 95, 1),
                                (2, 8, 95, 2),
